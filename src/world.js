...
export class GameWorld {
  ...
  getDiagnostics(){const player=this.racers[0];if(!player)return null;const velocity=player.body.linvel();const bodyForwardSpeed=Math.hypot(velocity.x,velocity.z);return{requestedSpeed:player.speed,actualForwardSpeed:player.actualForwardSpeed,bodyForwardSpeed,activeContacts:this.activePlayerContacts.size,forwardMovement:player.windowForwardMovement,stuckTimer:player.stuck,lateral:player.lateral,trackLimit:GAME.trackWidth/2,lives:this.playerLives,lastCollision:this.lastCollision,lastRecovery:this.lastRecovery};}
